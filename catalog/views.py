...
from catalog.serializers import CategorySerializer, ProductSerializer, \
    DiscountSerializer, SellerSerializer, AddProductSerializer, BasketSerializzer, Serializer, DeleteProductSerializer
...
class BasketView(APIView):
    ...
    def get(self, request):
        ...
        basket = Product.objects.prefetch_related('basket_set').filter(basket__user=user).values(
            "name", "price", "discount", count=F("basket__count"), discount_percent=F("discount__percent"),
            discount_date_end=F("discount__date_end"))
        serializer = BasketSerializzer({"products": basket})
        return Response(serializer.data)
